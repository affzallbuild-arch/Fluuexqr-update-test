(function($){
  ...
  function trackerKey(){ return keyBase() + '_v129_tracker'; }
  ...
  function fetchBill(){
    return $.ajax({ url: window.menuqr_ajax.ajax_url, method: 'POST', cache: false, data: requestData() });
  }
  ...
  function restoreMenu(){
    $('#menuqr-order-status-wrap,#menuqr-bill-history').attr('hidden','hidden').removeClass('active is-active').css('display','none').attr('aria-hidden','true');
  ...
  function showTrackerShell(){
    ...
    wrap.removeAttr('hidden').addClass('active is-active fqx-v130-status-open').css('display','block').attr('aria-hidden','false');
    $('body').addClass('fqx-v130-tracker-open');
    window.requestAnimationFrame(function(){ wrap.get(0) && wrap.get(0).scrollIntoView({block:'start', behavior:'smooth'}); });
  }
  function stepsHtml(status){
    ...
    return '<div class="status-steps fqx-v130-steps">' + steps.map(function(step, i){
  ...
  function renderTracker(data){
    ...
    $('#st-current').text(orders.length ? (orders.length + ' order tracking') : 'Live tracking');
    $('#st-steps').empty();

    if(!orders.length){
      $('#st-items').html('<div class="fqx-v130-empty"><span>📍</span><h4>No active order found</h4><p>Order tracking will stay here for 4 hours after you place an order.</p></div>');
      $('#st-details').html('');
      return;
    }
    ...
    $('#st-items').html(orders.map(function(order, index){
      ...
      var orderStatus = String(order.order_status || 'pending');
      var pay = normalizeStatus(order.bill_payment_status || billStatus || order.payment_status || 'unpaid');
      return '<article class="fqx-v130-order-card ' + (orderStatus === 'served' ? 'is-served' : '') + '">' +
        '<div class="fqx-v130-order-head"><div><strong>Order ' + orderNo + '</strong><span>' + esc(order.unique_code || ('MQR-' + order.id)) + '</span></div><b class="badge badge-' + esc(orderStatus) + '">' + esc(label(orderStatus)) + '</b></div>' +
    ...
    $('#st-details').html('<div class="fqx-v130-bill-card"><div><span>Running Bill Total</span><strong>' + money(bill ? bill.grand_total : 0) + '</strong></div><div><span>Payment Status</span><strong class="badge ' + badgeClass(billStatus) + '">' + esc(label(billStatus)) + '</strong></div>' + (billUrl ? '<a class="btn btn-primary btn-full" target="_blank" rel="noopener" href="' + esc(billUrl) + '">🧾 Open Real Bill</a>' : '') + '</div>');
  }
  function openTracker(){
    showTrackerShell();
    if(fetchingTracker){ return; }
    fetchingTracker = true;
    $('#st-items').html('<div class="section-card fqx-v130-loading">Loading your 4-hour order tracking…</div>');
    fetchBill().done(function(resp){ renderTracker(resp && resp.success ? (resp.data || {}) : {}); }).fail(function(){ $('#st-items').html('<div class="section-card">Tracking refresh failed. Please try again.</div>'); }).always(function(){ fetchingTracker = false; });
  }
  function refreshTrackerSilent(){
    if(!$('#menuqr-order-status-wrap').is(':visible') || fetchingTracker){ return; }
    fetchingTracker = true;
    fetchBill().done(function(resp){ if(resp && resp.success){ renderTracker(resp.data || {}); } }).always(function(){ fetchingTracker = false; });
  }
  ...
  function openBill(){
    $('#menuqr-order-status-wrap').attr('hidden','hidden').removeClass('active is-active').css('display','none');
    ...
    window.requestAnimationFrame(function(){ var el = document.getElementById('menuqr-bill-history'); if(el){ el.scrollIntoView({block:'start', behavior:'smooth'}); } });
  }
  ...
  $(document).off('click.fqxV130BackMenu', '[data-menuqr-back="menu"], #menuqr-close-bill-history').on('click.fqxV130BackMenu', '[data-menuqr-back="menu"], #menuqr-close-bill-history', function(e){ e.preventDefault(); e.stopImmediatePropagation(); $('body').removeClass('fqx-v130-tracker-open'); restoreMenu(); });
  $(document).off('click.fqxV130Refresh', '#menuqr-status-refresh').on('click.fqxV130Refresh', '#menuqr-status-refresh', function(e){ e.preventDefault(); refreshTrackerSilent(); });
  ...
  $(function(){
    cleanExpired(trackerKey()); cleanExpired(ordersKey()); cleanExpired(latestOrderKey());
    window.setInterval(function(){ refreshTrackerSilent(); if($('#menuqr-bill-history').is(':visible')){ fetchBill().done(function(resp){ if(resp && resp.success){ renderBill(resp.data || null); } }); } }, 9000);
  });
})(jQuery);
